...
| **Option source entity** *(optional)* | An `input_select` or `select` whose current state supplies the target option at each off-to-on transition. This lets Home Assistant automations, calendars, seasons, or any other logic decide the selection without duplicating that logic in MoLight |
| **Fixed/fallback option** *(optional with a source; otherwise required)* | The option to apply when no source is configured, or when the source is missing, unavailable, unknown, or does not match an option offered by the target. It can represent a preset, theme, mood, mode, or any integration-specific choice |
