...
- **Schedule** — with its **behavior** (`follow`, gate-and-turn-off, gate-and-switch-state, or gate-and-keep-state).
...
- **Scheduled → gated** — every Virtual Scheduled Light remains eligible. The inside-schedule profile becomes the regular Virtual Light settings, the shared schedule is retained as its gate, and the corresponding turn-off, switch-state, or keep-state behavior is selected. The outside-schedule profile is permanently discarded after an explicit confirmation warning.
...
For worked examples with the exact field values to enter — a plain turn-off timer, a single-sensor room, the full occupancy/illuminance/schedule setup, a porch light, and a day/night hallway light — see [EXAMPLES.md](EXAMPLES.md).
...
| **Schedule behavior** | Default `follow` — lights turn on at window start and off at window end (porch lights). Every Gate mode restricts automatic activation to the window. **Gate and turn off** forces off at its end. **Gate and switch state** does not force off solely because the window ended: it recalculates an on light's state and deadline from current illuminance and presence/door state, occupancy/maintain history, and the configured timeout, which can still make the light due to turn off. **Gate and keep state** preserves the current on-period, hold, countdown, warning, and timer deadline. Switch/keep both allow occupancy, maintain occupancy, and the door to hold or re-hold an already-on light outside the window; once off, the gate blocks a fresh automatic activation. |
...
- Settings no chosen light could apply are rejected at the form rather than stored and silently ignored: a fade when none of the lights support transitions, a color temperature or a color when none can show one. One capable light is enough — mixed groups are the point — and a light that hasn't reported its capabilities yet is never taken as proof, so the check can't block on incomplete information.
