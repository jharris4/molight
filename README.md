...
| **Schedule behavior** | Default `follow` — lights turn on at window start and off at window end (porch lights). Every Gate mode restricts automatic activation to the window. **Gate and turn off** forces off at its end. **Gate and switch state** keeps an on light on but recalculates its state and deadline from current sensor state/history and the configured timeout. **Gate and keep state** preserves the current on-period, hold, countdown, warning, and timer deadline. Switch/keep both allow occupancy and the door to hold or re-hold an already-on light outside the window; once off, the gate blocks a fresh automatic activation. |
...
On the schedule's `on → off` boundary, **Switch state** instead replaces an already-on light's running state/deadline using the outside profile. Active occupancy, maintain occupancy, or an open-and-close door adopts it as `OCCUPIED`. Otherwise the remaining timeout is calculated from the outside occupancy history; no sensor or usable history earns a fresh full timeout. If the resulting deadline is already due, the outside profile's effect/warning/automatic-off settings apply immediately.
