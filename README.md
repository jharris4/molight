...
| **Turn-on selection entity** *(optional)* | A `select` entity to set immediately before MoLight turns the lights on — for example, the preset select exposed by WLED. Also enter the exact option below |
| **Turn-on selection option** *(optional)* | The option to apply. It can represent a preset, theme, mood, mode, or any integration-specific choice. Used for both manual Virtual Light and automatic off-to-on commands; not reapplied for adjustments while already on, warning restores, or physical member-light turn-ons |
...
An optional **turn-on selection entity** and **turn-on selection option** let MoLight choose a preset, theme, mood, mode, or other integration-specific setting through a `select` entity before turning on. For example, WLED exposes its presets this way. MoLight selects the configured option first, waits for that service call to finish, and then turns on the member lights. This happens once per MoLight-commanded off-to-on transition, whether manual or automatic. A physical member-light turn-on is left alone because applying a selection afterward could overwrite an intentional external choice. If the select is unavailable or the option was renamed, MoLight logs a warning and still turns on the lights.
